    ...
    <DocArticle meta={diagnosticsMeta}>
        ...
        rows={[
          [
            ...
            <fbt desc="Docs table cell — diag 101">
              {
                "Unknown token reference (`color.primry`) — only fires for known theme groups, so ordinary dotted CSS values pass. Comes with a did-you-mean fix. Also used for forward references between theme tokens: define before use, and for an unknown `breakpoint.X` / `container-size.X` reference inside an `@media`/`@container` prelude."
              }
            </fbt>,
          ],
          [
      ...
      <DocH2>
        <fbt desc="Docs content — heading: diag component">
          {"Component structure (ARV120–ARV126, ARV129)"}
        </fbt>
      </DocH2>
      <DocTable
        headers={[
        ...
        rows={[
          ...
          [
            "ARV129",
            <fbt desc="Docs table cell — severity error">{"error"}</fbt>,
            <fbt desc="Docs table cell — diag 129">
              {
                "A `component`/`style` declared inside a component, recipe or style — they belong at the top level or in a top-level/`global` at-rule."
              }
            </fbt>,
          ],
        ]}
      />
      <DocH2>
        <fbt desc="Docs content — heading: diag modes">{"Theme modes (ARV130–ARV134)"}</fbt>
      </DocH2>
      <DocTable
        headers={[
          "Code",
