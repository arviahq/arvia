...
<div className={s.root} data-state={open ? "open" : "closed"}>
  ...
  <div className={s.panel}>{children}</div>
      ...
      <DocP>
        <fbt desc="Docs content — patterns: adaptive card lead-in">
          {
            "Everything at once — slots, a variant, nested container queries, and a recipe — in a component that stays correct in any column the layout offers:"
          }
        </fbt>
      </DocP>
      <DocCode
  ...
  base {
    use Surface;
    container-type: inline-size;   // measure this card's own width
    display: flex;
    flex-direction: column;
    overflow: hidden;
    ...
    @container (min-width: container-size.wide) {   // go horizontal once the card is ≥ wide
      flex-direction: row;
      media { aspect-ratio: auto; width: 240px; }
    }
  }
  ...
  variants {
    emphasis {
      ...
      featured {
        ...
        @container (min-width: container-size.wide) {
          media { width: 320px; }  // featured + wide earns bigger art
        }
      }
    }
  }
  ...
  defaults { emphasis: normal; }
      ...
      <DocP>
        <fbt desc="Docs content — patterns: adaptive card trace">
          {
            "Trace a card rendered in a 700px column: the `@container (min-width: container-size.wide)` query flips the layout to a row, and if the card is also `featured`, its own nested container query widens the media slot. Drag the same card into a 300px sidebar and all of it unwinds — no props, no resize observers."
          }
        </fbt>
      </DocP>
      <DocH2>
